...
const char* MODULE_ID = "tilt_sensor";
...
const int CALIBRATION_SAMPLES_COUNT = 20;
const int CALIBRATION_DELAY_MS = 50;
const int CALIBRATION_STABILITY_THRESHOLD_ADC = 18;

void send_error_message(const char* to_id, const char* error_type, const char* reason);
void send_data_message(float pitch, float roll);
...
void TiltSensor::reset_smoothing_filter() {
    ...
    if (smoothing_window_samples > 0 && smoothing_window_samples <= MAX_SMOOTHING_SAMPLES) {
        ...
        Serial.println(F("Warning: Invalid smoothing window size. Defaulting to 1."));
...
bool TiltSensor::calibrate_sensor(const char* requester_id) {
    ...
    SensorStateEnum prev_state = current_sensor_state; // Use the class's enum
    current_sensor_state = SS_CALIBRATING;

    long temp_sum_x_adc = 0, temp_sum_y_adc = 0; // Local sums for calibration process
    int min_x_adc = (int)ADC_MAX_VALUE + 1, max_x_adc = -1;
    int min_y_adc = (int)ADC_MAX_VALUE + 1, max_y_adc = -1;
    int current_raw_x_adc, current_raw_y_adc;

    for (int i = 0; i < CALIBRATION_SAMPLES_COUNT; i++) {
        get_raw_tilt_values_adc(current_raw_x_adc, current_raw_y_adc); // Call member function
        temp_sum_x_adc += current_raw_x_adc;
        temp_sum_y_adc += current_raw_y_adc;
    ...
    calib_offset_x_adc = (float)temp_sum_x_adc / CALIBRATION_SAMPLES_COUNT;
    calib_offset_y_adc = (float)temp_sum_y_adc / CALIBRATION_SAMPLES_COUNT;

    Serial.print(F("Calibration OK. Offset X_ADC: ")); Serial.print(calib_offset_x_adc);
    Serial.print(F(" Y_ADC: ")); Serial.println(calib_offset_y_adc);

    reset_smoothing_filter(); // Call member function
    current_sensor_state = (prev_state == SS_CALIBRATING ? SS_IDLE : prev_state);
    if (current_sensor_state == SS_IDLE && monitoring_active) { // If was monitoring, resume
...
bool TiltSensor::start_monitoring(bool perform_calibration, const char* requester_id) {
    ...
    if (perform_calibration) {
        ...
        Serial.println(F("INFO: Calibration required before starting. Attempting calibration."));
        if (!calibrate_sensor(requester_id)) { // Call member function
            Serial.println(F("Start FAILED (mandatory calibration failed). Sensor remains IDLE."));
            current_sensor_state = SS_IDLE;
            monitoring_active = false;
            return false;
        }
    ...
    if (!was_monitoring_locally) {
        Serial.println(F("Sensor monitoring started."));
    }
    return true;
}
...
void handle_config_message(const JsonDocument& doc, const char* from_id) {
    if (!doc.containsKey("name")) {
        ::send_error_message(from_id, "invalid_config", "config 'name' field missing");
        return;
    }
    ...
    if (strcmp(name, "number_sample") == 0) {
        ...
        Serial.println(F("Config: Calibration requested."));
...
void handle_json_message(const String& json_string) {
    ...
    if (!doc.containsKey("mtype") || !doc.containsKey("from") || !doc.containsKey("to")) {
        const char* from_id_on_error = doc.containsKey("from") ? doc["from"].as<const char*>() : "*";
        ::send_error_message(from_id_on_error, "missing_required_fields", "mtype, from, or to missing");
        return;
    }

    const char* to_field = doc["to"];
    if (strcmp(to_field, MODULE_ID) != 0 && strcmp(to_field, "*") != 0) {
        return; // Message not for this module
    }

    const char* mtype = doc["mtype"];
    const char* from_id = doc["from"];

    if (strcmp(mtype, "config") == 0) {
        handle_config_message(doc, from_id);
    } else if (strcmp(mtype, "tilt_sensor.start") == 0) {
        if (sensor.monitoring_active && sensor.current_sensor_state == TiltSensor::SS_MONITORING) {
             ::send_status_message(from_id, "command_ack", "tilt_sensor.start", "already_monitoring");
        } else if (sensor.start_monitoring(false, from_id)) { 
            ::send_status_message(from_id, "command_ack", "tilt_sensor.start", "monitoring_started");
        }
    } else if (strcmp(mtype, "tilt_sensor.stop") == 0) {
        sensor.stop_monitoring(from_id);
        ::send_status_message(from_id, "command_ack", "tilt_sensor.stop", "monitoring_stopped");
    }
}
...
void setup() {
    ...
    Serial.println(F("--- Tilt Sensor Module Initializing ---"));
    Serial.print(F("Module ID: ")); Serial.println(MODULE_ID);

    sensor.init(); // Initialize the sensor object's internal state

    if (sensor.start_monitoring(true, "*")) { 
        Serial.println(F("Module initialized and monitoring started."));
    } else {
        Serial.println(F("Module initialized. Monitoring NOT started (calibration may have failed)."));
    }
...
void send_data_message(float pitch, float roll) {
  ...
  doc["mtype"] = String(MODULE_ID) + ".data";
  doc["from"] = MODULE_ID;
  doc["to"] = "*";
